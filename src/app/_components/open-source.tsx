...
const openSourceContributions = [
  {
    ...
    projectDescription:
      "The proactive AI assistant that actually does (not ask) things.",
    contributionDescription:
      "Added Google Chat as a communication channel. Fixed hardcoded onboarding options.",
  ...
  },
  {
    ...
    projectDescription:
      "AI-powered git commit message, changelog & documentation generator using opencode",
    contributionDescription:
      "Added global config, configurable providers, PR creation and branch management.",
  ...
  },
  {
    ...
    contributionDescription:
      "Fixed server connection issue plaguing many users.",
...
async function fetchGitHubStars(githubUrl: string): Promise<number | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `Failed to fetch stars for ${owner}/${repo}: ${res.status}`
      );
      return null;
    }
...
function ContributionCard({
  ...
  contribution: (typeof openSourceContributions)[0];
  stars: number | null;
}) {
  return (
    <div className="flex flex-col gap-3 group py-4">
      ...
      <div className="flex items-center gap-4">
        ...
        {contribution.forkUrl && (
          ...
          >
            <svg
              className="w-4 h-4 icon icon-tabler icons-tabler-outline icon-tabler-git-fork"
...
export default async function OpenSourceContributions() {
  const contributionsWithStars = await Promise.all(
    openSourceContributions.map(async (contribution) => {
      const stars = await fetchGitHubStars(contribution.githubUrl);
      return { ...contribution, stars };
    })
  );
  ...
  return (
    <div className="flex flex-col gap-4 w-full border-t border-gray-200 dark:border-gray-700 pt-4">
      ...
      <div className="flex flex-col divide-y divide-gray-300 dark:divide-gray-700 divide-dashed">
        {contributionsWithStars.map((contribution, index) => (
          <ContributionCard
            key={index}
